...
CKPT = Path('cs_viz_cache/rd_curve50.pkl'); rates = []
if CKPT.exists():
...
# exclude degenerate images: prediction survives even the loose prior probe -> ~0 rate,
# so R(D) is trivially flat and normalizing by ~0 is undefined.  Report how many.
keep = R.max(1) > 500.0
n_excl = int((~keep).sum()); R = R[keep]; n = R.shape[0]
# normalized shape: each image / its OWN max rate (robust to MC-noise non-monotonicity)
Rn = R / (R.max(1, keepdims=True) + 1e-9)
mean_n, se_n = Rn.mean(0), Rn.std(0)/math.sqrt(n)
med, q1, q3 = np.median(R, 0), np.percentile(R, 25, 0), np.percentile(R, 75, 0)
print(f'[filter] kept {n} images, excluded {n_excl} degenerate (~0-rate, prior-robust)')
...
ax[0].set_title(f'R(D) shape averaged over {n} images ({n_excl} prior-robust excluded)',
...
plt.tight_layout(rect=[0,0,1,0.96]); out='cs_viz_outputs/rd_curve50.png'
plt.savefig(out, dpi=150, bbox_inches='tight'); plt.close()
drop = 100*(mean_n[0] - mean_n[-1])/mean_n[0]
print(f'saved {out}  | mean rate falls {drop:.0f}% from tau=0.02 to 0.70 (n={n}, {n_excl} excluded)')
